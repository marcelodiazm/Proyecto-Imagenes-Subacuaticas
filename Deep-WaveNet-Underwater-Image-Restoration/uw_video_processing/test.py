...
result_dir = '/content/drive/MyDrive/PDI/a_limpiar/Train_5_limpia/'
...
if __name__ =='__main__':
    ...
    file_name = 'video_yt _corto.mp4'
    #total_files = get_frames(file_name)
    output_frames = []
    total_files = os.listdir('/content/drive/MyDrive/PDI/a_limpiar/Train5_4/Train5_4_1/')
    st = time.time()
    with tqdm(total=len(total_files)) as t:

        for m in range(len(total_files)):
        
            img = cv2.imread('/content/drive/MyDrive/PDI/a_limpiar/Train5_4/Train5_4_1/'+total_files[m])
            img = img[:, :, ::-1]   
